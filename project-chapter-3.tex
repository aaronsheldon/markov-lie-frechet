...
\end{longtable}
\begin{algorithm}[!ht]
	\caption[Non-commutative Perturbation of $\frac{\partial e^X}{\partial x}$]{Numerical calculation of the non-commutative perturbation of the gradient of the matrix exponential $\frac{\partial e^X}{\partial x}$, using the $\left[ 13/14 \right]_f\left(x\right)$ Pad\'{e} approximation of $f\left(x\right)=\sum_{n=0}^\infty \frac{1}{\left(n+2\right)!} x^n$}
	\label{alg:perturbation}
	\begin{algorithmic}[1]
		\Function{PEX}{$X$, $\frac{\partial X}{\partial x}$}
			...
			\State $P \gets P_{13}\left(A_X\right)$\Comment{Pad\'{e} numerator by recursive summing and squaring}
			\State $Q \gets Q_{14}\left(A_X\right)$\Comment{Pad\'{e} denominator by recursive summing and squaring}
			\State Solve for $\vec{y}$: $P \vec{a_x} = Q \vec{y}$\Comment{Call to linear solver}
			\State \Return $\operatorname{MAT}\left(\vec{y}\right)$
		\EndFunction
	\end{algorithmic}
\end{algorithm}
\begin{algorithm}[!ht]
	...
	\label{alg:gradient}
	\begin{algorithmic}[1]
		\Function{GEX}{$X$, $\frac{\partial X}{\partial x}$}
			\State \Return $\left(\frac{\partial X}{\partial x} + \operatorname{PEX}\left(X,\frac{\partial X}{\partial x}\right)\right) \operatorname{EXP}\left(X\right)$
		\EndFunction
	\end{algorithmic}
\end{algorithm}
\begin{figure}[!ht]
...
\end{longtable}
\begin{algorithm}[!ht]
	\caption[Bilinear perturbation of $\frac{\partial^2 e^X}{\partial x \partial y}$]{Numerical calculation of the bilinear perturbation of the Hessian of the matrix exponential $\frac{\partial^2 e^X}{\partial x \partial y}$, using the $\left[ 12/14 \right]_f\left(x\right)$ Pad\'{e} approximation of $f\left(x\right)=\sum_{n=0}^\infty \frac{n+1}{\left(n+3\right)!} x^n$.}
	\label{alg:bilinear}
	\begin{algorithmic}[1]
		\Function{BEX}{$X$, $\frac{\partial X}{\partial x}$, $\frac{\partial X}{\partial y}$, $\epsilon = \text{machine float precision}$}
			\State $A_x \gets \left[X,\frac{\partial X}{\partial x}\right]$\Comment{Allocates memory}
			...
			\If{$A_x = 0$}
				...
				\State $P \gets P_{12}\left(A_X\right)$\Comment{Pad\'{e} numerator by recursive summing and squaring}
				\State $Q \gets Q_{14}\left(A_X\right)$\Comment{Pad\'{e} denominator by recursive summing and squaring}
				\State Solve for $\vec{z}$: $P \vec{a_y} = Q \vec{z}$\Comment{Call to linear solver}
				\State \Return $\left[\frac{\partial X}{\partial x}, \operatorname{MAT}\left(\vec{z}\right)\right]$
			\EndIf
			\If{$A_y = 0$}
				\State $A_X \gets I \otimes X - X^\dagger \otimes I$\Comment{if $X$ is $n \times n$ the result is $n^2 \times n^2$}
				\State $\vec{a_x} \gets \operatorname{VEC}\left(A_x\right)$\Comment{Change of indexing}
				...
				\State \Return $\left[\frac{\partial X}{\partial y}, \operatorname{MAT}\left(\vec{z}\right)\right]$
			\EndIf
...
\begin{algorithm}[!ht]
	...
	\label{alg:hessian}
	\begin{algorithmic}[1]
		\Function{HEX}{$X$, $\frac{\partial X}{\partial x}$, $\frac{\partial X}{\partial y}$, $\frac{\partial^2 X}{\partial x \partial y}$}
			\State $P_x \gets \frac{\partial X}{\partial x} + \operatorname{PEX}\left(X,\frac{\partial X}{\partial x}\right)$\Comment{Call to non-commutative perturbation}
			\State $P_y \gets \frac{\partial X}{\partial y} + \operatorname{PEX}\left(X,\frac{\partial X}{\partial y}\right)$\Comment{Call to non-commutative perturbation}
