...
\end{IEEEeqnarray*}
such that the first $n+m-1$ derivatives of the rational polynomial approximation evaluated 
at $x=0$ equal the first $n+m-1$ coefficients of the Taylor series, i.e.
\begin{IEEEeqnarray*}{rCl}
	\left.\frac{d^k}{d x^k}\frac{P_n\left(x\right)}{Q_m\left(x\right)}\right|_{x=0}
		& = & \frac{1}{\left(k+2\right)!} \text{ with } 0 \le k \le n+m-1.
\end{IEEEeqnarray*}
