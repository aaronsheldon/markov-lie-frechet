...
are displayed in table \ref{tab:padecoefficients}. The order of the Pad\'{e} approximation 
...
We can summarize this work in an algorithm to compute the non-commutative perturbation\ref{alg:noncommutativperturbation}.
\begin{algorithm}
	\caption[Non-commutative Perturbation]{Numerical calculation the non-commutative perturbation of the gradient of the matrix exponential $\frac{\partial e^X}{\partial x}$.}
	\label{alg:noncommutativperturbation}
	\begin{algorithmic}[1]
		\Function{PER}{$X$,$dX$}
...
numerical stability, and using squaring methods to efficiently compute the matrix 
polynomials. A point of concern is the first assignment of the Kronecker product, which 
results in a quadratic increase in the amount of memory used. Finally, an algorithm to 
compute the gradient of the matrix exponential can be formulated\ref{alg:gradient}.
\begin{algorithm}
	\caption[Gradient]{Numerical calculation of the gradient of the matrix exponential $\frac{\partial e^X}{\partial x}$.}
	\label{alg:gradient}
	\begin{algorithmic}[1]
		\Function{DEX}{$X$,$dX$}
...
In the next step of developing the Hessian we find the Taylor series for the bilinear
...
\begin{IEEEproof}
	Apply the previous lemmas in the order they were stated to the symmetric sum 
	of the two terms.
	\begin{IEEEeqnarray*}{rCl}
		\IEEEeqnarraymulticol{3}{l}
		{
...
\begin{longtable}{r r r}
	\caption{Pad\'{e} Approximation Coefficients}
	\label{tab:padecoefficients}\\
