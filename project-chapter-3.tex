...
\begin{IEEEeqnarray*}{rCl}
	\frac{\partial e^X}{\partial x}
		& = & \operatorname{mat}\left(\sum_{n=0}^{\infty} \frac{1}{\left(n+2\right)!} \left(I \otimes X - X^T \otimes I \right)^{n+1} \operatorname{vec}\left(\frac{\partial X}{\partial x}\right)\right) e^X + \frac{\partial X}{\partial x} e^X
\end{IEEEeqnarray*}

And thus can be approximated by
\begin{IEEEeqnarray*}{rCl}
	\frac{\partial e^X}{\partial x}
		& \approx & \operatorname{mat}\left(\frac{P_n\left(I \otimes X - X^T \otimes I\right)}{Q_m \left(I \otimes X - X^T \otimes I\right)} \left(I \otimes X - X^T \otimes I \right)\operatorname{vec}\left(\frac{\partial X}{\partial x}\right) \right) e^X + \frac{\partial X}{\partial x} e^X
\end{IEEEeqnarray*}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Function{PER}{$X$,$D_x$}
			...
			\Else
				\State $A_X \gets I \otimes X - X^T \otimes I$\Comment{if $X$ is $n \times n$ the result is $n^2 \times n^2$}
...
\begin{algorithm}[!hb]
	...
	\begin{algorithmic}[1]
		\Function{HEX}{$X$,$D_x$,$D_y$,$D_{xy}$}
			\State $E \gets e^X$\Comment{Allocates memory}
			\State $Z \gets D_{xy} + \operatorname{PER}\left(X,D_{xy}\right)$\Comment{Allocates memory}
			\State $Z \gets Z + \frac{1}{2} \operatorname{BIL}\left(X,D_x,D_y\right)$\Comment{In place computation}
			\State $P_x \gets D_x + \operatorname{PER}\left(X,D_x\right)$\Comment{Allocates memory}
			\State $P_y \gets D_y + \operatorname{PER}\left(X,D_y\right)$\Comment{Allocates memory}
