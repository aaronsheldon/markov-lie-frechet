...
reside in the associated Lie algebra. 
...
\begin{IEEEeqnarray*}{rCl}
	M^{\left(t\right)}
		...
		\end{bmatrix}
\end{IEEEeqnarray*}
is a valid stochastic matrix, as $M^{\left(t\right)} \hat{\mathbbm{1}} = \hat{\mathbbm{1}}$
for all $t \ge 1$. But it is not a Markov process because at $t = \ln 2$ the $\det M^{\left(t\right)} = 0$,
so that the dual vector $\left[-1,1\right]$ representing the parity statistic vanishes
\begin{IEEEeqnarray*}{rCl}
	\left[-1,1\right] M^{\left(t\right)}
		& = & \left[-1,1\right]
			\begin{bmatrix}
				e^{-t} & 1 - e^{-t} \\
				1 - e^{-t} & e^{-t}
		...
		& = & 0
\end{IEEEeqnarray*}
In the context of Lie theory, the path $t$ is not closed in the Lie algebra.
