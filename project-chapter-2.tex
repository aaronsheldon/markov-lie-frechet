...
the connect algebraic closure and differentiation, with the norm implicitly used in the
...
% ToDo: Fill in the blanks of the vertexes
...
\begin{IEEEproof}
	...
	\begin{enumerate}
		...
		\item It follows that any convex sum of the vertexes $V_{j\left(\cdot\right)}$ belongs
		to the convex polytope of singly stochastic matrices.
		...
		such that $f^{\left(1\right)}\left(i\right)=j$ and $m_{k f^{\left(1\right)}\left(k\right)} > 0$ for all other $k \ne i$.
		...
		\begin{IEEEeqnarray*}{rCl}
			M^{\left(1\right)} \hat{\mathbbm{1}}
				...
				& = & \hat{\mathbbm{1}}
		\end{IEEEeqnarray*}
		\item Next we check to ensure that $\left\langle \hat{e}_k, M^{\left(1\right)} \hat{e}_l \right\rangle > 0$
		for every $k,l$
		\begin{IEEEeqnarray*}{rCl}
			\left\langle \hat{e}_k, M^{\left(1\right)} \hat{e}_l \right\rangle
				...
				& \ge & 0
		\end{IEEEeqnarray*}
		because $1 > m_{ij}$, or else we are done; and $m_{kl} \ge m_{ij}$ for all $k,l$ by 
		construction.
		\item Finally, observe that $\left\langle \hat{e}_i, M^{\left(1\right)} \hat{e}_j \right\rangle = 0$, 
		so that we can carry out finite induction on this process, at most $n^2$ times. This 
		will generate matrices $M^{\left(t\right)}$ and $V_{f^{\left(t\right)}\left(\cdot\right)}$, 
...
powerful commutator relationship. To get there we start with a calculation of the products
of vertex dual matrices.
...
Furthermore in the case where $j\left(\cdot\right)$ is a permutation we can relate powers of 
$C_{j\left(\cdot\right)}$ to compositions of $j\left(\cdot\right)$.

\begin{corollary}
	If $j\left(\cdot\right)$ is a permutation then for $n>0$ we have $\left(I + C_{j\left(\cdot\right)}\right)^n = I + C_{j^n\left(\cdot\right)}$
\end{corollary}

\begin{IEEEproof}
	Starting with $n = 1$ carry through with induction...
\end{IEEEproof}

Decompositions of $j$ results in a linear decomposition of $C$...a decomposition of $j$ is a
partition such that...equivalent to non-communicating states...$j$ can be decomposed into
cyclic permutations and transients...unique binary partition into a transient set and a
permutation.

\begin{IEEEeqnarray*}{rCl}
	V_{j\left(\cdot\right)} & = & \exp\left(\alpha_0 C_{j_t\left(\cdot\right)} + \sum_{n=1}^{p-1} \alpha_n C_{j_p^n\left(\cdot\right)} \right)
\end{IEEEeqnarray*}

The vertex is recovered with $\alpha_0 \rightarrow \infty$, and the Pythagorean coefficients
