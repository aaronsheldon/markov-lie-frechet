\chapter{The Lie Algebra Continuous Time Markov Processes on Finite State Spaces}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		$A \notin GL\left(n\right)$, again satisfying closure within $GL\left(n\right)$.
	\end{enumerate}
\end{proof}
	...
	\begin{enumerate}
		...
		\begin{IEEEeqnarray*}{rCl}
			\exp\left(\alpha C_{ij} + \beta C_{ji}\right)
				...
				& = & I + \frac{1 - e^{-\alpha - \beta }}{\alpha + \beta} \left(\alpha C_{ij} + \beta C_{ji}\right)
		\end{IEEEeqnarray*}
	\end{enumerate}
\end{proof}
...
any state $j \ne i$ is given by $G_i = \sum_{i \ne j} \alpha_j C_{ij}$ (proof to 
follow in chapter four), from which we have
...
\begin{lemma}
	\begin{IEEEeqnarray*}{rCl}
		...
		\begin{cases}
			...
			C_{jin} - C_{jjn} - 2C_{iin} + C_{lln} & i=k,\\
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\begin{IEEEeqnarray*}{rCl}
			C_{ijn}C_{iln}
				& = & C_{jin} - C_{jjn} - C_{iin} + C_{lln} - C_{iin}\\
				& = & C_{jin} - C_{jjn} - 2C_{iin} + C_{lln}
		\end{IEEEeqnarray*}
		\item When $i=l$
		\begin{IEEEeqnarray*}{rCl}
			C_{ijn}C_{kin}
...
% the following needs major fixing for i=l and j=k
\begin{corollary}
	\begin{IEEEeqnarray*}{rCl}
	...
	\begin{enumerate}
		...
		\begin{IEEEeqnarray*}{rCl}
			\left[C_{ijn},C_{iln}\right]
				...
				& = & C_{jin} - C_{jjn} - 2C_{iin} + C_{lln} - C_{lin} + C_{lln} + 2C_{iin} - C_{jjn}\\
				& = & C_{jin} - C_{lin} - 2C_{jjn}  + 2C_{lln}
		\end{IEEEeqnarray*}
		\item When $i=l$
		\begin{IEEEeqnarray*}{rCl}
		...
		\begin{IEEEeqnarray*}{rCl}
			\left[C_{ijn},C_{kjn}\right]
				...
				& = & C_{jkn} - C_{ijn} + C_{iin} - C_{jjn} - C_{kkn} - C_{jin} + C_{kjn} - C_{kkn} + C_{jjn} + C_{iin}\\
				& = & C_{jkn} - C_{ijn} + 2C_{iin} - 2C_{kkn} - C_{jin} + C_{kjn}
		\end{IEEEeqnarray*}
		\item When none of the conditions apply
		\begin{IEEEeqnarray*}{rCl}
			\left[C_{ijn},C_{kln}\right]
