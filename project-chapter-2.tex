	...
	\begin{enumerate}
		...
		\begin{IEEEeqnarray*}{rCl}
			C_{j_E\left(\cdot\right)} C_{j_T\left(\cdot\right)}
				...
				& = & 0
		\end{IEEEeqnarray*}
		\item $\left(\alpha C_{j_T\left(\cdot\right)} + \beta C_{j_E\left(\cdot\right)}\right)^n = \sum_{m=0}^n \alpha^{m}\beta^{n-m} C^m_{j_T\left(\cdot\right)} C^{n-m}_{j_E\left(\cdot\right)}$
	\end{enumerate}
\end{observation}

By construction $j_E\left(\cdot\right)$ is a permutation, and at the very least has a 
period $p > 0$; the permutation may also contain sub-cycles. As well there is a $t>n$ such 
that for ever $i \in T_{j\left(\cdot\right)}$ we have $j^t_T\left(i\right) \in E_{j\left(\cdot\right)}$.
Thus to find a limit in the stochastic Lie algebra $\mathfrak{st}(\hat{\mathbbm{1}})$ we 
need only consider a coefficient $\alpha_0$ for $C_{j_T\left(\cdot\right)}$ and $p-1$
...
\begin{IEEEeqnarray*}{rCl}
	V_{j\left(\cdot\right)} 
		& = & \exp\left(\alpha_0 C_{j_T\left(\cdot\right)} + \sum_{n=1}^{p-1} \alpha_n C_{j_E^n\left(\cdot\right)} \right)\\
		& = & \sum_{m=0}^\infty \frac{1}{m!} \left( \alpha_0 C_{j_T\left(\cdot\right)} + \sum_{n=1}^{p-1} \alpha_n C_{j_E^n\left(\cdot\right)} \right)^m
\end{IEEEeqnarray*}
