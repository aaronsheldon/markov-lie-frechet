...
$\hat{e}_i$; so that $M\hat{\mathbbm{1}} = \hat{\mathbbm{1}}$ and $\left\langle \hat{e}_i, M \hat{e}_j \right\rangle \ge 0$
for all $i,j$. The vertexes of the convex polytope of nonnegative stochastic matrices with 
respect to $\hat{e}_i$ can be formulated as linear combinations of the generators $C_{ij}$
of $\mathfrak{st}(\hat{\mathbbm{1}})$.
...
Every vertex matrix is uniquely dual to matrix in $\mathfrak{st}(\hat{\mathbbm{1}})$ 
...
Furthermore in the case where $j\left(\cdot\right)$ we can relate powers of $C_{j\left(\cdot\right)}$
to powers of $j\left(\cdot\right)$.
...
Note that $\mathfrak{st}^{+}(\hat{\mathbbm{1}})$ is a real vector space, but that it is
augmented, through outer sum with the group of integers $\mathbb{Z}$. This is because every
real point has added to it every multiple of $\mathrm{i}2\pi$.
