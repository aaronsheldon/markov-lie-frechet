...
stochastic matrices are always characterized with respect to a specific unit vector, which
we will denote by $\hat{\mathbbm{1}}$, for reasons that will become clear later. In the next
two sections we provide an explicit construction and characterization of the Lie algebra of
stochastic matrices, building on the original the work of Johnson\cite{johnson_markov-type_1985},
and distilling the recent works of Sumner et al.\cite{sumner_lie_2012,fernandez-sanchez_lie_2012}, 
Mourad\cite{mourad_lie-theoretic_2004}, and Chruściński et al.\cite{chruscinski_pseudo-stochastic_2015}.

The common approach to stochastic matrices begins with the restriction that the matrices
have non-negative entries with respect to the standard orthonormal basis for the vector
...
with state transitions illustrated in figure \ref{fig:singlestochastic}, as holding
significance in the structure of $\mathfrak{st}(\hat{\mathbbm{1}})$. In fact this will be
the central result of this chapter: that the algebraic closure of the matrices $C_{ij}$ is
the stochastic Lie algebra $\mathfrak{st}(\hat{\mathbbm{1}})$. To establish this result we
need a preliminary result that proves the commutators $\left[C_{ij},C_{kl}\right]$ are
...
are not the generators, or logarithms, of the vertexes $V_{j\left(\cdot\right)}$. In fact, 
...
\begin{IEEEproof}
	...
	\begin{enumerate}
		...
		where starting with an $M$ in the convex polytope of singly stochastic matrices we 
		eliminate nonzero entries from $M$ with convex sums involving $V_{j\left(\cdot\right)}$.
		\item Begin by picking an $i,j$ such that $m_{ij} = \left\langle \hat{e}_i, M \hat{e}_j \right\rangle$
...
The vertex dual matrices $C_{j\left(\cdot\right)}$ are algebraically closed, and have a
powerful commutator algebra. We start with a calculation of the products of vertex dual
matrices.
\begin{corollary}
...
\begin{IEEEproof}
	...
	\begin{IEEEeqnarray*}{+rCl+x*}
		\left(I + C_{j\left(\cdot\right)}\right)^{n \bmod p}
...
we define a binary partition of the set $\left\lbrace 1,\dots,n \right\rbrace$; composed of
respectively the numbers for which $j\left(\cdot\right)$ is a permutation, and the numbers
for which $j\left(\cdot\right)$ is transient.
\begin{definition}
	The ergodic and transient subsets $E_{j\left(\cdot\right)}, T_{j\left(\cdot\right)} \subseteq \left\lbrace 1,\dots,n \right\rbrace$,
	defined by $j\left(\cdot\right)$, are respectively:
	\begin{IEEEeqnarray*}{rCl}
		E_{j\left(\cdot\right)} 
			& = & \left\lbrace i : \exists p_i > 0 \text{ where } j^{p_i}\left(i\right)=i \right\rbrace\\
		T_{j\left(\cdot\right)} 
			& = & E^c_{j\left(\cdot\right)}
	\end{IEEEeqnarray*}
\end{definition}

The partitioning into an ergodic subset and a transient subset allows us to construct a pair
of restriction maps of $j\left(\cdot\right)$.
\begin{definition}
	...
	\begin{IEEEeqnarray*}{rCl}
		j_E\left(i\right) 
			& = &
			\begin{cases}
				j\left(i\right) & i \in E_{j\left(\cdot\right)}\\
				i & \text{otherwise}
			\end{cases}\\
		j_T\left(\cdot\right) 
			& = &
			\begin{cases}
				j\left(i\right) & i \in T_{j\left(\cdot\right)}\\
				i & \text{otherwise}
			\end{cases}
	\end{IEEEeqnarray*}
\end{definition}

Nearly without any further proof we can state the following observations
\begin{observation}
	The following relationships for $j\left(\cdot\right)$, $j_E\left(\cdot\right)$, and $j_T\left(\cdot\right)$
	\begin{enumerate}
		\item $i \in E_{j\left(\cdot\right)} \Rightarrow j_E\left(i\right) \in E_{j\left(\cdot\right)}$
		\item $\exists i \in T_{j\left(\cdot\right)} : j_T\left(i\right) \in E_{j\left(\cdot\right)}$
		\item $j\left(i\right) = j_T\left(j_E\left(i\right)\right)$
		\item $C_{j\left(\cdot\right)} = C_{j_E\left(\cdot\right)} + C_{j_T\left(\cdot\right)}$
		\item $\operatorname{im}\left(C_{j_T\left(\cdot\right)}\right) \leqslant \ker\left(C_{j_E\left(\cdot\right)}\right)$
		which follows from
		\begin{IEEEeqnarray*}{rCl}
			C_{j_E\left(\cdot\right)} C_{j_T\left(\cdot\right)}
				& = & C_{j_T\left(j_E\left(\cdot\right)\right)} - C_{j_E\left(\cdot\right)} - C_{j_T\left(\cdot\right)}\\
				& = & C_{j\left(\cdot\right)} - C_{j_E\left(\cdot\right)} - C_{j_T\left(\cdot\right)}\\
				& = & C_{j_E\left(\cdot\right)} + C_{j_T\left(\cdot\right)} - C_{j_E\left(\cdot\right)} - C_{j_T\left(\cdot\right)}\\
				& = & 0
		\end{IEEEeqnarray*}
	\end{enumerate}
\end{observation}

The last observation admits a further generalization that will be of use in the following
calculations
\begin{observation}
	If $\left[A,B\right]= AB$ then
	\begin{enumerate}
		\item $\left(A+B\right)^n=\sum_{m=0}^n A^m B^{n-m}$
		\item $\exp\left(A+B\right)=\sum_{n,m} \frac{1}{\left(n+m\right)!}A^mB^n$
	\end{enumerate}
\end{observation}

By construction $j_E\left(\cdot\right)$ is a permutation, and at the very least has a 
period $P > 0$; the permutation may also contain sub-cycles. As well, there is a $T>0$ such 
that for each $i \in T_{j\left(\cdot\right)}$ is absorbed by $j^t_T\left(i\right) = i_T \in E_{j\left(\cdot\right)}$
for all $t > T$. Thus to find a limit in the stochastic Lie algebra $\mathfrak{st}(\hat{\mathbbm{1}})$
we need only consider a coefficient $\alpha_0$ for $C_{j_T\left(\cdot\right)}$ and $p-1$
coefficients $\alpha_n$ for the $p-1$ vertex dual matrices $C_{j^n_E\left(\cdot\right)}$. We
propose a trial solution for $V_{j\left(\cdot\right)}$, to which we apply the previous 
observations to simply.
\begin{IEEEeqnarray*}{rCl}
	V_{j\left(\cdot\right)} 
		& = & \exp\left(\alpha_0 C_{j_T\left(\cdot\right)} + \sum_{l=1}^{p-1} \alpha_l C_{j_E^l\left(\cdot\right)} \right)\\
		& = & \sum_{n,m}\frac{1}{\left(n+m\right)!} \left(\alpha_0 C_{j_T\left(\cdot\right)}\right)^m \left(\sum_{l=1}^{p-1} \alpha_l C_{j_E^l\left(\cdot\right)}\right)^n
\end{IEEEeqnarray*}

The vertex is recovered with $\alpha_0 \rightarrow \infty$, and the Pythagorean
coefficients.
\begin{IEEEeqnarray*}{rCl}
	\alpha_n 
		& = & 
		\begin{cases}
			\left(-1\right)^{n+1}\frac{\pi}{p}\csc\left(\frac{n\pi}{p}\right)e^{\mathrm{i}\frac{n\pi}{p}} & n \text{ even}\\
			\left(-1\right)^{n+1}\frac{\pi}{p}\csc\left(\frac{n\pi}{p}\right) & n \text{ odd}
		\end{cases}
\end{IEEEeqnarray*}

The Pythagorean coefficients are so named because $\frac{\pi}{p}\csc\left(\frac{\pi}{p}\right)$ 
is ratio of $\pi$ to the $p$ inner Pythagorean approximation of $\pi$.
...
Note that $\mathfrak{st}^{+}(\hat{\mathbbm{1}})$ is a real vector space, but it is 
augmented with the group of integers $\mathbb{Z}$. This is because every real point has 
added to it every multiple of $\mathrm{i}2\pi$.
...
stochastic matrices we can state immediately that the vertexes of the Birkhoff polytope are 
generated by:

\begin{IEEEeqnarray*}{rCl}
	V_{j\left(\cdot\right)} & = & \exp\left(\sum_{n=1}^{p-1} \alpha_n C_{j^n\left(\cdot\right)} \right)
\end{IEEEeqnarray*}

Where $p$ is the period of the permutation $j\left(\cdot\right)$. The vertex is recovered
with the Pythagorean coefficients.

\begin{IEEEeqnarray*}{rCl}
	\alpha_n & = & \left(-1\right)^{n+1}\frac{\pi}{p}\csc\left(\frac{n\pi}{p}\right)
\end{IEEEeqnarray*}

when $p$ is odd, and

\begin{IEEEeqnarray*}{rCl}
	\alpha_n & = & \left(-1\right)^{n+1}\frac{\pi}{p}\csc\left(\frac{n\pi}{p}\right)e^{\mathrm{i}\frac{n\pi}{p}}
\end{IEEEeqnarray*}

when $p$ is even.

This particular logarithm is not unique, nor is it even the principle branch in general.
Other logarithms can be quickly generated by further decomposing the permutation $j\left(\cdot\right)$
